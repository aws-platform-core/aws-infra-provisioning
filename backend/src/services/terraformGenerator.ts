...
if (!backendBucket || !backendLockTable || !backendRegion) {
  throw new Error(
    "Missing TF_BACKEND_BUCKET, TF_BACKEND_LOCK_TABLE, or TF_BACKEND_REGION"
  );
}

type GenerateStackInput = {
  requestId: string;
  provider: string;
  templateId: string;
  moduleSource: string;
...
function inferTerraformType(value: unknown): string {
  if (typeof value === "string") return "string";
  if (typeof value === "number") return "number";
  if (typeof value === "boolean") return "bool";
  if (Array.isArray(value)) return "list(any)";
  if (typeof value === "object" && value !== null) return "map(any)";
  return "any";
}

function buildModuleAssignments(parameters: Record<string, unknown>): string {
  ...
  const maxKeyLength = variableNames.reduce(
...
function buildVariableBlocks(parameters: Record<string, unknown>): string {
  const variableNames = Object.keys(parameters);
...
export function generateTerraformFiles(input: GenerateStackInput) {
