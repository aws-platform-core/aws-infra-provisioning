type CostBreakdownItem = {
  ...
  function estimateS3Cost(parameters: Record<string, unknown>): CostEstimateResponse {
    const storageGb =
      typeof parameters.estimated_storage_gb === "number"
        ? parameters.estimated_storage_gb
        : 100;
  
    const putRequests =
      typeof parameters.estimated_monthly_put_requests === "number"
        ? parameters.estimated_monthly_put_requests
        : 100000;
  
    const getRequests =
      typeof parameters.estimated_monthly_get_requests === "number"
  ...
  function estimateEc2Cost(parameters: Record<string, unknown>): CostEstimateResponse {
    const instanceType =
      typeof parameters.instance_type === "string"
